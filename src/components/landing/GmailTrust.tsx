"use client";

import { motion } from "framer-motion";
import { Shield, Eye, Lock, ToggleRight } from "lucide-react";
import BlurReveal from "@/components/shared/BlurReveal";

const EASE = [0.16, 1, 0.3, 1] as const;

const TRUST_QA = [
  {
    icon: Eye,
    q: "¿Leen mis correos personales?",
    a: "No. Neto filtra SOLO correos de BCP, BBVA, Interbank, Scotiabank, Yape y Plin. Tu inbox personal es 100% invisible.",
  },
  {
    icon: Lock,
    q: "¿Pueden acceder a mi cuenta bancaria?",
    a: "Jamás. Leemos notificaciones de correo, no nos conectamos a tu banco. No pedimos contraseñas bancarias.",
  },
  {
    icon: ToggleRight,
    q: "¿Puedo desconectarlo?",
    a: "Sí, en 1 clic. Google Settings → Apps de terceros → Revocar. Tus datos se borran en 24 horas.",
  },
  {
    icon: Shield,
    q: "¿Es seguro?",
    a: "Usamos Google OAuth 2.0, el mismo sistema que usan Notion, Slack y Figma. Datos encriptados con TLS 1.3.",
  },
];

const TECH_BADGES = ["Google OAuth 2.0", "TLS 1.3", "Supabase RLS"];

export default function GmailTrust() {
  return (
    <section className="py-24 relative overflow-hidden bg-gradient-to-b from-transparent via-neto-green/[0.02] to-transparent">
      <div className="max-w-4xl mx-auto px-6">
        {/* Header */}
        <BlurReveal>
          <div className="text-center mb-12">
            <span className="rounded-full border border-neto-green/30 bg-neto-green/10 px-4 py-1.5 text-xs font-semibold tracking-wide text-neto-green uppercase mb-5 inline-block">
              Privacidad
            </span>
            <h2 className="text-3xl min-[860px]:text-5xl font-extrabold tracking-tight leading-[1.1] mb-4 text-neto-txt">
              ¿Te preocupa conectar tu correo?{" "}
              <span
                className="text-transparent"
                style={{ WebkitTextStroke: "1.5px rgba(229,226,222,0.7)" }}
              >
                Es normal.
              </span>
            </h2>
            <p className="text-neto-txt3 text-base max-w-[540px] mx-auto leading-relaxed">
              Te explicamos exactamente qué hace Neto con tu Gmail — y qué NO puede hacer.
            </p>
          </div>
        </BlurReveal>

        {/* Q&A grid */}
        <div className="grid grid-cols-1 min-[640px]:grid-cols-2 gap-4 mb-10">
          {TRUST_QA.map((item, i) => {
            const Icon = item.icon;
            return (
              <motion.div
                key={item.q}
                className="rounded-[20px] bg-neto-bg2/60 border border-white/[0.07] p-6 backdrop-blur-sm"
                initial={{ opacity: 0, y: 16 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, margin: "-40px" }}
                transition={{ duration: 0.5, delay: i * 0.07, ease: EASE }}
              >
                <div className="w-8 h-8 rounded-lg bg-neto-green/15 flex items-center justify-center mb-4">
                  <Icon size={16} className="text-neto-green-light" />
                </div>
                <p className="text-sm font-semibold text-neto-txt mb-2">{item.q}</p>
                <p className="text-sm text-neto-txt3 leading-relaxed">{item.a}</p>
              </motion.div>
            );
          })}
        </div>

        {/* Tech badges */}
        <div className="flex flex-wrap gap-2 justify-center mb-8">
          {TECH_BADGES.map((badge) => (
            <span
              key={badge}
              className="rounded-full border border-neto-bg5 bg-neto-bg3 px-4 py-1.5 text-xs font-medium text-neto-txt2"
            >
              {badge}
            </span>
          ))}
        </div>

        {/* Fallback note */}
        <motion.p
          className="text-center text-sm text-neto-txt3 leading-relaxed max-w-xl mx-auto rounded-2xl border border-white/5 bg-neto-bg3/40 p-4"
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          transition={{ duration: 0.5, delay: 0.3, ease: EASE }}
        >
          ¿Prefieres no conectar tu correo? Sin problema.{" "}
          <span className="text-neto-txt2">
            Puedes registrar gastos manualmente por WhatsApp — la experiencia básica funciona perfecto sin Gmail.
          </span>
        </motion.p>
      </div>
    </section>
  );
}
